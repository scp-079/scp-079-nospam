...
def is_from_user(_, __, message: Message) -> bool:
    ...
    try:
        # TODO TEMP fix
        if message.from_user and message.from_user.id not in {777000, 1087968824}:
            return True
    except Exception as e:
        ...
        if not (text or image_path):
            ...
            if forward_name and forward_name not in glovar.except_ids["long"]:
                ...
                if (message.forward_from_chat
                        ...
                        and is_regex_text("fcnm", message.forward_from_chat.username)):
                    return f"ban from {message.forward_from_chat.username}"
